...
window.addEventListener('scroll', function() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (e) {
            ...
            // 入力確認
            console.log("フォーム送信:", { name, email, subject });

            // ここに実際の送信処理を追加（サーバーAPI等に送信）
            // 現在はテスト用のアラート表示
            alert(`メッセージを受け付けました。\n名前: ${name}\nメール: ${email}\n件名: ${subject}`);
    ...
    function enhanceFormValidation() {
        ...
        if (contactForm) {
            ...
            if (!nameFeedback || !emailFeedback || !subjectFeedback || !messageFeedback) {
                console.error('フィードバック要素が見つかりません');
                return;
            }
            ...
            nameInput.addEventListener('blur', function () {
                validateInput(this, nameFeedback, 'お名前を入力してください', this.value.trim() !== '');
            });

            emailInput.addEventListener('blur', function () {
                validateInput(this, emailFeedback, '有効なメールアドレスを入力してください', validateEmail(this.value.trim()));
            });

            subjectInput.addEventListener('blur', function () {
                validateInput(this, subjectFeedback, '件名を入力してください', this.value.trim() !== '');
            });

            messageInput.addEventListener('blur', function () {
                validateInput(this, messageFeedback, 'メッセージを入力してください', this.value.trim() !== '');
            });
        }
    }
